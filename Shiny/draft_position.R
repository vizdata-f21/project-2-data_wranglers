all_stars_by_draft <- function(year_start = 1951,
                               year_end = 2021) {
...
#all_stars_n$z <- stringr::str_split(all_stars_n$draft_pick, "Rnd 1 Pick ") %>%

#unnest_wider(all_stars_n, z) %>%
#  mutate(draft = ifelse(as.numeric('...2') <= 10, T, F) ~ "Top 10")
